...
if active_page == "Corrosion Data":
    ...
    st.caption(
        "Generate one macro-enabled source-first workbook containing "
        "every site linked to the selected source. Existing corrosion "
        "observations are included under each site, with one starter row "
        "for new observations. Use the + control in Excel to add as many "
        "additional observation rows as needed."
    )
    ...
    if not corrosion_workbook_available_sources:
        ...
        st.caption(
            ...
            "will be included · "
            "1 new-observation starter row will be provided per linked site."
        )

        st.info(
            "The downloaded workbook is macro-enabled. "
            "Open it in desktop Microsoft Excel and enable macros/content "
            "when prompted so that the + row-duplication controls work. "
            "The workbook can still be viewed without enabling macros, "
            "but the + controls will not function."
        )
    ...
    st.write("### Import completed Excel workbook")
    ...
    uploaded_corrosion_workbook = st.file_uploader(
        "Upload completed corrosion workbook",
        type=["xlsm", "xlsx"],
        key="corrosion_observation_excel_upload",
        help=(
            "Use a source-first corrosion workbook generated by "
            "this curator. Existing observation IDs update existing "
            "records; rows without IDs create new observations. "
            ".xlsm is the current format; older .xlsx workbooks are "
            "also accepted."
        ),
    )

    corrosion_excel_preview_df = pd.DataFrame()

    if uploaded_corrosion_workbook is not None:
        try:
            raw_corrosion_excel_df = (
                read_corrosion_entry_workbook(
                    uploaded_corrosion_workbook
                )
            )

            if raw_corrosion_excel_df.empty:
                st.info(
                    "No completed corrosion observation rows "
                    "were found. Blank source/site rows were "
                    "ignored."
                )

            else:
                existing_corrosion_site_ids = (
                    get_existing_site_ids()
                )

                existing_corrosion_source_codes = {
                    normalise_source_code(
                        value
                    )
                    for value
                    in get_existing_source_codes()
                    if str(
                        value or ""
                    ).strip()
                }

                existing_corrosion_site_source_pairs = {
                    (
                        str(
                            site_id
                        ).strip(),
                    ...
                    in get_existing_site_source_pairs()
                }

                current_corrosion_observations = (
                    get_corrosion_observations()
                )

                corrosion_excel_preview_df = (
                    validate_corrosion_workbook_rows(
                        raw_corrosion_excel_df,
                ...
                with summary_col1:
                    st.metric(
                        "Ready",
                        corrosion_ready_count,
                    )

                with summary_col2:
                    st.metric(
                        "Errors",
                        corrosion_error_count,
                    )

                with summary_col3:
                    st.metric(
                        "New",
                        corrosion_create_count,
                    )

                with summary_col4:
                    st.metric(
                        "Updates",
                        corrosion_update_count,
                    )
                ...
                preview_display_columns = [
                    column
                    for column
                    in preview_display_columns
                    if column
                    in corrosion_excel_preview_df.columns
                ]
                ...
                if corrosion_error_count > 0:
                    st.error(
                        "The workbook contains validation errors. "
                        "Correct the highlighted rows in Excel and "
                        "upload the workbook again. Import is disabled "
                        "while any error remains."
                    )

                else:
                    confirm_corrosion_excel_import = st.checkbox(
                        (
                            "I reviewed the Excel corrosion preview "
                            "and want to create/update these observations."
                        ),
                        key="confirm_corrosion_excel_import",
                    )

                    corrosion_excel_import_clicked = st.button(
                        "Confirm Excel corrosion import",
                        key="confirm_corrosion_excel_import_button",
                        type="primary",
                    )

                    if corrosion_excel_import_clicked:
                        if not confirm_corrosion_excel_import:
                            st.error(
                            ...
                            for (
                                _,
                                preview_row,
                            ) in corrosion_excel_preview_df.iterrows():
            ...
            st.error(
                "Could not read or validate the corrosion "
                f"workbook: {exc}"
            )        
